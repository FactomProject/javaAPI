...
public class Entry {
	...
	public void setExtIDs(String[] extIDs) {
		Integer eidlen=extIDs.length;
		 ...
		 for (int i=0 ; i < extIDs.length ;i++) {
			 if (extIDs[i].equals(null)) {
				 //no entry.  skip it
				 item=new byte[0];
			 }  else if (extIDs[i].length() < 4){
				 item=extIDs[i].getBytes();	
			 } else if (extIDs[i].substring(0,4).equals("HEX:")){
				 item=utils.hexToBytes(extIDs[i].substring(4));
			 } else {
				 item=extIDs[i].getBytes();				 
			 }
	...
	public void setEntry(String json) {
		ChainID=utils.hexToBytes(utils.getJSONValue(json, "chainid"));
		Content=utils.hexToBytes(utils.getJSONValue(json, "content"));
		JSONArray jsonExts=utils.getJsonArray(json,"extids");
		byte[][] extids=new byte[jsonExts.length()][];
		for (int i=0;i<jsonExts.length();i++) {
			try {
				extids[i]=utils.hexToBytes(jsonExts.getString(i));
